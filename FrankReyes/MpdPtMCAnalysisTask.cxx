...
void MpdPtMCAnalysisTask::UserInit()
{
      ...
      dedx_protonescut = new TH2F("h__dedx_Protons_cut", "dedx vs P for the Protons also cuts;p*q, GeV/c;dE/dx, arb. units", 300, -1.5, 1.5, 600, 0.0, 60);
      fOutputList->Add(dedx_protonescut);
      ptcut_protones = new TH1F("PT_protones_cut", "Pt Protons cuts; Pt; Entries", 300, 0, 5);
      fOutputList->Add(ptcut_protones);
      // RECO CORTES

      dedx_pionescut = new TH2F("h__dedx_Pions_cut", "dedx vs P for the Pions also cuts;p*q, GeV/c;dE/dx, arb. units", 300, -1.5, 1.5, 600, 0.0, 60);
      fOutputList->Add(dedx_pionescut);
      ptcut_piones = new TH1F("PT_pions_cut", "Pt Pions cuts; Pt; Entries", 300, 0, 5);
      fOutputList->Add(ptcut_piones);

      dedx_kaonescut = new TH2F("h__dedx_Kaons_cut", "dedx vs P for the Kaons also cuts;p*q, GeV/c;dE/dx, arb. units", 300, -1.5, 1.5, 600, 0.0, 60);
      fOutputList->Add(dedx_kaonescut);
      ptcut_kaones = new TH1F("PT_Kaons_cut", "Pt Kaons cuts; Pt; Entries", 300, 0, 5);
      fOutputList->Add(ptcut_kaones);

      // RECO CON CORTES Y ID

      iddedx_protonescut = new TH2F("h__dedx_Protons_cut_id", "dedx vs P for the Protons also cuts and ID;p*q, GeV/c;dE/dx, arb. units", 300, -1.5, 1.5, 600, 0.0, 60);
      fOutputList->Add(iddedx_protonescut);
      idptcut_protones = new TH1F("Pt_Protones_cut_id", "Pt Protons cuts id; Pt; Entries", 300, 0, 5);
      fOutputList->Add(idptcut_protones);

      iddedx_pionescut = new TH2F("h__dedx_Pions_cut_id", "dedx vs P for the Pions also cuts and ID;p*q, GeV/c;dE/dx, arb. units", 300, -1.5, 1.5, 600, 0.0, 60);
      fOutputList->Add(iddedx_pionescut);
      idptcut_piones = new TH1F("Pt_Pions_cut_id", "Pt Pions cuts id; Pt; Entries", 300, 0, 5);
      fOutputList->Add(idptcut_piones);

      iddedx_kaonescut = new TH2F("h__dedx_Kaons_cut_id", "dedx vs P for the Kaons also cuts and ID;p*q, GeV/c;dE/dx, arb. units", 300, -1.5, 1.5, 600, 0.0, 60);
      fOutputList->Add(iddedx_kaonescut);
      idptcut_kaones = new TH1F("Pt_Kaons_cut_id", "Pt Kaons cuts id; Pt; Entries", 300, 0, 5);
      fOutputList->Add(idptcut_kaones);

      // MC

      pt_protones_mc = new TH1F("Pt_Protons_MC", "Pt Protons cuts MC; Pt; Entries", 300, 0, 5);
      fOutputList->Add(pt_protones_mc);

      pt_piones_mc = new TH1F("Pt_Pions_MC", "Pt Pions cuts MC; Pt; Entries", 300, 0, 5);
      fOutputList->Add(pt_piones_mc);

      pt_kaones_mc = new TH1F("Pt_Kaons_MC", "Pt Kaons cuts MC; Pt; Entries", 300, 0, 5);
      fOutputList->Add(pt_kaones_mc);
...
void MpdPtMCAnalysisTask::ProcessEvent(MpdAnalysisEvent &event)
{
      ...
      for (Int_t i = 0; i < NumTracksMC; i++)
      {
            ...
            else if (abspdgcode == 321)
            {
                  // Kaonplus (kp)
                  pt_kaones_mc->Fill(pt_mc);
            }
            else if (abspdgcode == 211)
            {
                  // pionplus (pip)
      ...
      for (long int i = 0; i < ntr; i++)
      {
            ...
            if (RECOmID == 1 && mctrack->GetMotherId() == -1)
                  continue; // primary tracks
            // if(RECOmID == 2 && mctrack -> GetMotherId() != -1) continue; //secondary tracks

            //------------------------------------------------------LIMITES PARA PID-----------------------------------------------------------------------------------

            Double_t proton_max = ((-1.30554) / TMath::Power(p / TMath::Sqrt(p * p + 0.88), 1.52176)) * ((0.381237 - TMath::Power(p / TMath::Sqrt(p * p + 0.88), 1.52176)) - (TMath::Log(1.46136 + TMath::Power(1.0 / (p / 0.9383), 1.50261))));
            Double_t proton_min = (-1.33806 / TMath::Power(p / TMath::Sqrt(p * p + 0.88), 0.76431)) * ((0.895334 - TMath::Power(p / TMath::Sqrt(p * p + 0.88), 0.76431)) - (TMath::Log(1.95555 + TMath::Power(1.0 / (p / 0.9383), 2.40083))));

            Double_t pi_max = ((-0.694978) / TMath::Power(p / TMath::Sqrt(p * p + 0.01949), 6.0991)) * (((-0.544038) - TMath::Power(p / TMath::Sqrt(p * p + 0.01949), 6.0991)) - (TMath::Log((-0.221145) + TMath::Power(1.0 / (p / 0.1396), (-0.380694)))));
            Double_t pi_min = ((-0.481401) / TMath::Power(p / TMath::Sqrt(p * p + 0.01949), (6.88153))) * (((-0.65936) - TMath::Power(p / TMath::Sqrt(p * p + 0.01949), (6.88153))) - (TMath::Log((-0.37918) + TMath::Power(1.0 / (p / 0.1396), (-0.316248)))));

            Double_t kaon_max = ((-0.954003) / TMath::Power(p / TMath::Sqrt(p * p + 0.2437), (2.39503))) * (((-0.0273105) - TMath::Power(p / TMath::Sqrt(p * p + 0.2437), (2.39503))) - (TMath::Log((0.868322) + TMath::Power(1.0 / (p / 0.4937), (0.605659)))));
            Double_t kaon_min = ((-1.14442) / TMath::Power(p / TMath::Sqrt(p * p + 0.2437), (0.119658))) * (((0.403622) - TMath::Power(p / TMath::Sqrt(p * p + 0.2437), (0.119658))) - (TMath::Log((1.29185) + TMath::Power(1.0 / (p / 0.4937), (2.9028)))));
            //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
